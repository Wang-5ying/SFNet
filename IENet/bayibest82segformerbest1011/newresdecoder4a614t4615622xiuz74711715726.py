...
from second_model.IENet.bayibest82.baseapi.newapii711715  import BasicConv2d,node,CA,GM2  #atttention
...
class AM2(nn.Module):
    ...
    def forward(self, x1, x2, x4, x5):
        ...
        r3 = F.interpolate(r3, scale_factor=2)
        # print("hhhhhbao")
...
class M(nn.Module):
    ...
    def __init__(self, mode='small'):
        ...
        self.am = AM2(64, 320, 64, 128)
        self.u4 = nn.Conv2d(96, 1, 1)
        self.gm1 = GM2(64, 64, 96)
        self.gm2 = GM2(320, 320, 24)

        self.am_d = AM2(64, 320, 64, 128)
        self.u4_d = nn.Conv2d(96, 1, 1)
        self.gm1_d = GM2(64, 64, 96)
        self.gm2_d = GM2(320, 320, 24)
        ...
        self.node1 = node(64, 128, 96, 48)
        self.node2 = node(320, 512, 24, 12)

        self.gcn1 = Gru(64,64)
        self.gcn2 = Gru(64, 64)

        # self.u5 = nn.Conv2d(96*2, 1, 1)
    def forward(self, r, d, temperature):
        rte = self.resnet.extract_feat(r)

        ###############################T###################################
        dte = self.resnet.extract_feat(d)
        # print("dlayer_outputshhhh", dte[0].size(), dte[1].size(), dte[2].size(),
        #       dte[3].size())

        rd1, rd2 = self.node1(rte[0], rte[1], dte[0], dte[1],
                              temperature)
        rd3, rd4 = self.node2(rte[2], rte[3], dte[2], dte[3],
                              temperature)

        rgm1 = self.gm1(rd1, rte[0], temperature)
        rgm2 = self.gm2(rd3, rte[2], temperature)
        res1 = self.am(rgm1, rgm2, rte[0], rte[1])
        res1 = self.u4(res1)
        res1 = F.interpolate(res1, 384)

        tgm1 = self.gm1_d(rd1, dte[0], temperature)
        tgm2 = self.gm2_d(rd3, dte[2], temperature)
        res2 = self.am_d(tgm1, tgm2, dte[0], dte[1])  # !!!!!!
        res2 = self.u4_d(res2)
        res2 = F.interpolate(res2, 384)

        res = res1 + res2

        sr = F.interpolate(self.s1(self.student.forward(r)), size=384)

        bound1 = F.interpolate(self.bound1(rgm1), size=384)
        bound3 = F.interpolate(self.bound3(tgm1), size=384)
        contour1 = F.interpolate(self.bound2(rgm2), size=384)
        contour2 = F.interpolate(self.bound4(tgm2), size=384)  ####726

        return res, res1, res2, sr, bound1, bound3, contour1, contour2  ####726

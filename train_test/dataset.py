...
def cv_random_flip(img, label, ti,bound,body,detail):
    ...
    return img, label, ti,bound,body,detail


def randomCrop(image, label, ti,bound,body, detail):
    border = 30
    ...
    return image.crop(random_region), label.crop(random_region), ti.crop(random_region),bound.crop(random_region),\
           body.crop(random_region),detail.crop(random_region)


def randomRotation(image, label, ti,bound, body,detail):
    mode = Image.BICUBIC
    ...
    return image, label, ti,bound, body,detail
...
class SalObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root, ti_root, bound_root,body_root,detail_root,trainsize):
        ...
        self.tis = [ti_root + f for f in os.listdir(ti_root) if f.endswith('.jpg')
                       or f.endswith('.png')]
        self.bounds = [bound_root + f for f in os.listdir(bound_root) if f.endswith('.jpg')
                    or f.endswith('.png')]
        self.bodys = [body_root + f for f in os.listdir(body_root) if f.endswith('.jpg')
                       or f.endswith('.png')]
        self.details = [detail_root + f for f in os.listdir(detail_root) if f.endswith('.jpg')
                       or f.endswith('.png')]
    ...
    def __getitem__(self, index):
        ...
        image, gt, ti ,bound, body, detail = cv_random_flip(image, gt, ti,bound, body,detail)
        image, gt, ti ,bound, body, detail = randomCrop(image, gt, ti,bound, body,detail)
        image, gt, ti,bound , body, detail = randomRotation(image, gt, ti,bound, body,detail)
        ...
        detail = self.bound_transform(detail)
        return image, gt, ti,bound, body, detail
...
def get_loader(image_root, gt_root, ti_root,bound_root, body_root, detail_root, batchsize, trainsize, shuffle=True, num_workers=4, pin_memory=False):
    dataset = SalObjDataset(image_root, gt_root, ti_root,bound_root,body_root, detail_root, trainsize)
